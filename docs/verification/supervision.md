...
The blocking and bounded-follow-up mechanisms were validated across eight harnesses on 2026-07-08 through 2026-09-17, with Copilot CLI 1.0.86-2's native `agentStop` path and attached watcher continuation verified on 2026-09-17.
...
| Copilot | 1.0.86-2 | Native blocking `agentStop` hook | `FM_COPILOT_LIVE_E2E=1 bin/fm-test-run.sh tests/fm-copilot-primary-live-e2e.test.sh` passed session-start context, watcher-arm denial, one bounded stop continuation, watcher completion wake/drain/acknowledgement, and inert unrelated notifications on 2026-09-17. |
...
| Copilot | `FM_COPILOT_LIVE_E2E=1 bin/fm-test-run.sh tests/fm-copilot-primary-live-e2e.test.sh` | The current CLI completed the attached watcher task, resumed the model, drained and acknowledged the wake exactly once, and ignored an unrelated attached shell completion. |
...
The live Copilot notification carried the current snake-case `notification_type` and named shell id; `tests/fm-copilot-harness.test.sh` also pins the earlier camel-case `notificationType`, FirstMate title casing, exact title/message correlation, and the single-use root/home-bound receipt.
