...
| `fm-session-lock-lib.sh` | Shared session-lock ownership predicate built on `fm-harness-process-lib.sh` |
...
| `fm-windows-process-lib.sh` | Git Bash bridge to read native Windows process ownership and liveness without signaling MSYS process ids |
| `fm-windows-process.ps1` | Read-only native Windows process evidence provider for the shared identity owner |
...
| `fm-claude-hook-launch.ps1` | Windows-native Git Bash launcher for tracked Copilot and Claude-compatible hooks |
| `fm-private-path-lib.sh` | Unix-mode or native-Windows-ACL privacy validation for Firstmate-owned receipt paths |
