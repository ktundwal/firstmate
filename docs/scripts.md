...
| `fm-windows-process-lib.sh` | Route generation-bound Windows process ownership through `fm-windows-process.ps1` |
| `fm-private-path-lib.sh` | Validate protected data with Unix modes or the read-only `fm-windows-private-path.ps1` native ACL predicate |
