...
Native Windows with Git Bash remains a bounded Copilot/Herdr pilot, not a claim that every Unix lifecycle path works on Windows.
Use a native PowerShell executable and Git Bash rather than WSL Bash, and provision a private `FM_HOME` before starting; the [Copilot adapter reference](../.agents/skills/harness-adapters/references/harness/copilot.md) routes native ownership, launch, and ACL requirements.
The [Windows verification record](verification/runtime-backends.md#native-windows-copilot-pilot) distinguishes the exercised worker lifecycle from primary-hook, active-cancellation, and server-crash coverage that is still missing.
