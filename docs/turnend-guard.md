...
- Copilot registers `agentStop` in `.github/hooks/fm-primary.json`; `bin/fm-copilot-hook.sh` calls the shared guard with `--copilot` and translates exit 2 into Copilot's native `decision: "block"` object.
