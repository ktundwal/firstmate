...
It is not deterministic across the verified adapters: for example, codex, grok, gemini, and copilot depend on harness-private resume state or a session id, opencode continues the most recent session for the cwd, and claude, pi, pi-signed, omp, kimi, and agy have no verified pane-resume contract.
...
- `tests/fm-control-relaunch.test.sh` - the relaunch transaction: identity preservation, harness switching and retirement of the previous harness's per-task wiring, the progress note, checkpoint refusals, and rollback after a failed launch.
