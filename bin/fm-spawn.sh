...
#   /updatefirstmate, restart). A bare adapter name (claude|codex|copilot|opencode|pi|pi-signed|grok|kimi|cursor|gemini|muse|rovo|omp|agy)
...
launch_template() {
  ...
  codex)
    ...
    ;;
  # Copilot's interactive flag executes the launch instructions while retaining
  # the interactive session. The broad allow flags are the unattended-worker
  # contract; folder trust remains session-only and is handled after launch.
  copilot) printf '%s' 'env -u CLAUDECODE -u CLAUDE_PROJECT_DIR -u PI_CODING_AGENT -u GROK_AGENT -u FM_PI_HARNESS -u FM_OMP_HARNESS -u GEMINI_CLI -u ATLASSIAN_AGENT_TYPE -u ROVODEV_CLI -u CURSOR_AGENT -u CURSOR_INVOKED_AS copilot __COPILOTUPDATEFLAG__--allow-all --no-ask-user __MODELFLAG____EFFORTFLAG__-i "$(__OPINPUT__ encode launch-brief < __BRIEF__)"' ;;
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  if [ -r "/proc/$$/winpid" ]; then
    treehouse_bin=$(command -v treehouse) || {
      echo "error: Treehouse is not available to the Windows launcher" >&2
      exit 1
    }
    treehouse_shell=$(cygpath -w "$BASH") || {
      echo "error: could not resolve the Git Bash executable for Treehouse" >&2
      exit 1
    }
    treehouse_get_command="SHELL=$(shell_quote "$treehouse_shell") $(shell_quote "$treehouse_bin") get"
    if [ -n "${TREEHOUSE_ROOT:-}" ]; then
      treehouse_get_command="$treehouse_get_command --root $(shell_quote "$TREEHOUSE_ROOT")"
    fi
  fi
  spawn_send_text_line "$WT_TARGET" "$treehouse_get_command" || {
    echo "error: could not submit Treehouse acquisition to $WT_TARGET" >&2
    exit 1
  }
...
if [ "$KIND" != secondmate ]; then
  ...
  case "$HARNESS" in
  claude* | copilot* | opencode* | pi | pi-signed | omp)
...
COPILOTUPDATEFLAG=
if [ "$HARNESS" = copilot ] && [ -r "/proc/$$/winpid" ]; then
  COPILOTUPDATEFLAG='--no-auto-update '
fi
LAUNCH=${LAUNCH//__COPILOTUPDATEFLAG__/$COPILOTUPDATEFLAG}
...
case "$HARNESS" in
claude | codex | copilot | opencode | pi | pi-signed | grok | kimi | gemini | muse | rovo | agy)
...
if [ "$HARNESS" = copilot ] && [ -r "/proc/$$/winpid" ]; then
  LAUNCH="PATH=$(shell_quote "$PATH") $LAUNCH"
fi
