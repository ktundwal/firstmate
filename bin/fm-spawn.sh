...
launch_template() {
  ...
  case "$harness" in
    ...
    copilot) printf '%s' 'env -u CLAUDECODE -u CLAUDE_PROJECT_DIR -u PI_CODING_AGENT -u GROK_AGENT -u FM_PI_HARNESS -u FM_OMP_HARNESS -u GEMINI_CLI -u ATLASSIAN_AGENT_TYPE -u ROVODEV_CLI -u CURSOR_AGENT -u CURSOR_INVOKED_AS copilot --no-auto-update --allow-all --no-ask-user __MODELFLAG____EFFORTFLAG__-i "$(__OPINPUT__ encode launch-brief < __BRIEF__)"' ;;
...
spawn_current_path() {  # <target>
  case "$BACKEND" in
    ...
    herdr) fm_backend_herdr_current_path "$1" shell-discovery ;;
