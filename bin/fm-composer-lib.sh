...
#                plus `▀` bottom rule. A blank `┃` is empty only inside the
#                complete shape and with live Copilot identity.
...
_fm_composer_scan_screen() {  # <plain-screen> <cursor-or-empty> [extract-wrap]
  ...
  while IFS= read -r line; do
    ...
    # Copilot half-box: complete, width-matched rules around contiguous
    # left-bar content. A lone blank `┃` is never enough to prove a composer.
...
_fm_composer_leftbar_conflicts_halfbox() {  # <plain-screen> <first-row>
  ...
  while [ "$row" -ge 0 ]; do
    ...
    trimmed=$raw
    fm_composer_normalize_trim_var trimmed
    if [ -z "$trimmed" ]; then row=$((row - 1)); continue; fi
...
_fm_composer_halfbox_verdict() {  # <screen> <styled> <has-identity> <identity>
  ...
  agent=${identity%%$'\t'*}
  if [ "$agent" = copilot ]; then printf 'empty'; else printf 'unknown'; fi
}
